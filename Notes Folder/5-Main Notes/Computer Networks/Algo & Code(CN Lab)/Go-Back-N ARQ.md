...
Tags: #cn #gobackn #reliability_protocol 

# Go-Back-N ARQ

### Main Idea
- It's a more efficient method built on top of stop and wait where instead of receiving an acknowledgement for every single packet you send a bunch of packets (window size).
- And receive an acknowledgement for that bunch. If any packet is lost in that bunch then the whole thing is re-transmitted. 
